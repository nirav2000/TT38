...
function nextQuestion() {
  ...
  challengeArea.innerHTML = `
    ...
    <form id="answerForm">
      <input id="answerInput" type="number" inputmode="numeric" required autocomplete="off" />
  ...
  answerInput.addEventListener('blur', () => setTimeout(keepAnswerFocus, 0));
}
...
function trendDelta(records, metricGetter) {
  ...
  const recentValue = metricGetter(recent);
  const olderValue = metricGetter(older);
  return recentValue - olderValue;
}
...
function refreshAnalytics() {
  ...
  records.forEach((r) => {
    if (!summary[r.table]) summary[r.table] = { table: r.table, total: 0, ok: 0, speedSum: 0 };
  ...
  const ranked = Object.values(summary)
    .map((r) => ({ ...r, accuracy: r.total ? r.ok / r.total : 0, speed: r.total ? r.speedSum / r.total : 0 }))
